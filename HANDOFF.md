...
## 财务网站生产恢复与旧库/上传验收（2026-08-21，Codex，已上线）

Brain 中的财务网站仓库已完成实际修复，不再只是只读评审。代码已合并并推送到该仓库 `master`
（`7bd18de`），生产域名 `https://finance.hotcrush.net` 当前指向 Ready 部署
`dpl_5j6LgEe3bBSD7yXu8VynhzBsVQBN`。真实已登录浏览器会话下，鉴权、营收、财务和导入历史 API 均为 200；
最终部署的伪造会话请求稳定返回预期 401，而不是函数 500 或数据库连接超时。

已完成：数据库客户端与 Production URL 强制 TLS 加密（`sslmode=require`）；修正集成门禁中已删除的
`app_user_role` 旧断言并把共享库安全检查限定到财务站拥有对象；浏览器按 UTF-8 序列化后的真实字节数做
4 MiB 上传预检；重写 Vercel sensitive 环境变量；关闭实测不可用的外部 AI（Groq 403、Gemini 超时），
保留本地确定性规则引擎；推进配置缓存版本。`npm test` 501/501、静态门禁和真实旧库 `--all` 全通过。
本轮没有数据库 DDL，也没有改写财务事实数据。

需要继续保持诚实边界：旧审计把 `vercel env run` 返回的 sensitive 空占位符误判为“环境变量为空”，
现已更正；这类值必须用新部署运行结果验证，不能靠读取秘密值判断。2026-06/07 正式财务构件缺失、
2026-05 勾稽差异、成本卡缺价、旧原始 Excel 转换、原 Excel/PDF 的 finance raw Storage 留存仍未解决。
标准上传 handler 的事务/回滚与线上启动路径已验证，但没有权威新模板，因此未伪造一次业务写入来制造
“生产上传成功”证据。外部 Agent 增强也必须等有效 provider 凭据通过生产连通性测试后再开启。
...
两份 `.env` 均没有 `R6_*` 运行开关或密钥。本轮没有修改 `.env`、Vercel、旧应用 systemd 或 `DATABASE_URL`；
...
结构化 POS worker 已具备文件大小/SHA-256 校验、来源合同、专用 pipeline claim、可逆 quarantine/
restore、平台健康快照、单日和 1–31 日范围对账。范围 CLI 默认 dry-run，只有显式 `--apply` 才写 R6；
旧库只读事务在任何 R6 写入前结束，逐日 batch 可幂等重跑。`hotcrush-pos-worker --drain --max-runs N`
...
新增 `brainctl auto` 作为 Brain 新 PDF 的安全增量入口：每次重新 hash/classify，只把本次新增且
