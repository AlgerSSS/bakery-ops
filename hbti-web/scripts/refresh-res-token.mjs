...
// 浏览器会话里借出来的令牌。失效后顾客照样能登录、能答题（登录走 RES H5 每请求现取
// 的访客令牌，不碰这个后台令牌），但**答完题发券会失败**，`/api/health` 也会因为
// 券模板只读探测 401 而返回 503。
//
// 失效机制（2026-08-02 两轮实验实测，此前「约 24 小时自然失效」的说法已证伪）：
// 它是**空闲超时**，不是固定寿命 —— 空闲 1h/2h 仍可用，4h/6h/8h 全部 401；
// 每 15 分钟调用一次的同龄令牌 8 小时全程可用。
// 生产上已由服务器端 keepalive.sh（每 30 分钟）保活，正常不该再需要手动刷新；
// 这个脚本留给本地调试和服务器保活/轮换都失灵时的应急。
// 根治仍然是向 RES 申请受限服务凭证。
//
// 用法：
