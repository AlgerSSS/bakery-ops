...
## 三、R6 线上部署：已完成（2026-08-21）

`./scripts/deploy-r6-finance.sh` 执行成功，退出码 0。四步全过：
隔离守卫 → 应用 `20260821002900` → 注册 2 个 Raw 批次 → worker 装载 2 个 run → 对账。

**线上对账 `ok: true`，14 项检查零 mismatch**，七个域行数与金额全部等于旧生产：

| 域 / 口径 | 行数（旧库 = R6） | 金额（旧库 = R6） |
|---|---:|---:|
| PL_METRIC / STORE | 66 | 11,201,859.98 |
| EXPENSE / STORE | 103 | 2,504,579.87 |
| LABOR / STORE | 148 | 4,493,182.25 |
...
R6 线上现状：545 条月度事实（476 STORE + 69 GROUP，6 个域，2025-12 → 2026-07）、
成本卡 471 / 344 / 430 / 2453、三类孤儿引用均为 0、`fin_revenue_day` 0 行（源表本就为空）。

部署后完整远端验收 `accept-r6-platform.sh remote` 通过：迁移对齐与 lint、平台健康不变量、
Lark RAG 不变量、东京 timer/worker、全历史 POS 对账 `mismatchCount: 0`、
BakeryOps 带页码引用检索 —— 财务域的加入没有影响 R6 已有的任何能力。

## 四、仍未做，需要单独授权

1. **财务站仍连旧库。** 本轮不改 `DATABASE_URL`、不改 Vercel 变量 —— 与既有约束一致。
   R6 现在具备承接财务域的结构和数据，但切换本身是另一个决定。
2. **2026-04-12 旧库修正未执行**（见第一节）。修正它是对生产财务事实的写入。
