#!/bin/bash
# 一键部署到 tokyo-01（Vultr 东京）—— 2026-08-05 Contabo 62.72.46.80 失联后的替代生产机。
# 用法：
#   ./deploy-tokyo.sh          两端都部署(默认)
#   ./deploy-tokyo.sh core     只部署 tokyo-01(核心:复盘/预测/Lark/res_api)
#   ./deploy-tokyo.sh mac      只重启 Mac(WhatsApp/招聘)
#   ./deploy-tokyo.sh --skip-gate [target]   跳过门禁(快速迭代,慎用)
#
# 与旧 deploy.sh 的差别只有三处，其余逻辑与注释刻意保持一致：
#   1) 目标换成 tokyo-01，密钥换成 ~/.ssh/xray_tokyo
#   2) bakery-ops 不再装 Playwright 浏览器 —— 云上跑 INSTANCE_ROLE=core，
