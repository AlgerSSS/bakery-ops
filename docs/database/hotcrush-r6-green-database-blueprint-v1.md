...
- POS 6 日范围回填证明 5 个可信日与 1 个异常日可以在同一命令中分别发布/隔离，重跑复用同一 batch，范围对账 0 差异。
- POS 全历史正式 CLI 完成 260 日分窗迁移：229 个可信日、2699 个小时事实、31 个异常证据；独立九窗 verifier 的 mismatch 为 0。
- 全历史完成态 12 日复跑所有对象均复用、Worker `processed=0`；2026-02-15 回滚抽样证明 current 可退回且版本不删除。
...
- 2025-12-03 至 2026-08-19 的全历史迁移已完成：260 个 legacy batch、229 条 current 日事实、2699 条 current 小时事实、31 个隔离日，九窗 mismatch 为 0。
- 2026-08-20 在旧源已成为可处理日，但未进入 R6；这个端到端差值反证了当前只是固定截止日的历史快照，不是实时或定时增量同步。
